...
async function getTeeTimes_jcgolf(
  ...
  try {
    ...
    const tokenResp = await getJCToken(websiteId, bookingPrefix);
    const bearerToken = tokenResp.token;
    const transactionId = tokenResp.transactionId;
    ...
    // console.log("JC Golf Response:", response.data);
    if (response.status === 200) {
