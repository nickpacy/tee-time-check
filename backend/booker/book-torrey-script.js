...
async function loginUser(driver, email, password, courseClass) {
    await driver.get(`https://foreupsoftware.com/index.php/booking/19347/${courseClass}#/login`);
...
async function navigateToBookingPage(driver, courseClass) {
    await driver.get(`https://foreupsoftware.com/index.php/booking/19347/${courseClass}#/teetimes`);
...
function getMillisecondsUntil7PM() {
    ...
    console.log(`Waiting until 7 PM. The delay is ${delayInMinutes} minutes and ${remainingSeconds} seconds.`);
...
async function bookTeeTime(defaultStartTime, courseClass) {
    let driver = await initializeDriver();
    try {
        await loginUser(driver, 'nickpacy@gmail.com', 'Nichola$9', courseClass);
        await navigateToBookingPage(driver, courseClass);
